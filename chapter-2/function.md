...
`if __name__ == '__main__':` 这个作为脚本文件运行的入口，告诉 Python 解释器从这里开始运行代码，脚本运行的时候会依次执行 `if __name__ == '__main__':` 下面的代码，这个主要入口在编程中称为 mian，其他编程语言也有类似的机制。
...
#### 总结
function 可以帮助我们解决一类问题，除此之外 function 还可以做其他事，例如出现重复的操作，功能类似的代码，我们应该把这类东西抽象到一个 function 中解决，代码重复是非常难以维护的，我们一定不要写重复的代码。

...
前面我们学习了 list 这种数据类型，list 其实属于数据结构的一种，list 的特性是元素有序，可以增、删、改、通过下标查询，list 适合表示有序的数据集合，而有序是数据与数据之间的关系，学习数据结构的意义就是要学会如何用合适的数据结构去组织数据，表示数据。
...
相信读者会想 tuple 存在的意义是什么，tuple 相对与 list 来说是要省计算机内存的，所以如果你的数据集合是有序并且不会改变的话，用 tuple 来表示数据比用 list 来表示更加的合适。
...
说了这么多的 dict 的方法，其实我只是说了我经常用到的特性，dict 还有许多的其他方法，编程的世界有时候需要我们发挥联想能力和触类旁通的能力，比如 list 有各种处理数据的方法，我们就应该可以想象 dict 是不是也应该有类似的方法，我们要用 dict 实现我们没有见过的功能，是不是到 google 或者官方 API 文档搜索会找到相应的实现，而这样的想象力可以延伸到以后你学的任何技术中，很多时候你的灵机一动说不定就解决你遇到的问题了。
...
### 总结
这里我们已经将 Python 的常用数据结构学完了，正如我之前所说的编程需要我们发挥想象力，Python 其实还有别的数据结构在 [collections](https://docs.python.org/2/library/collections.html)，有序字典(OrderedDict)，双向列表(deque)，这些东西最终回归我们的主题，要使用合适的数据结构来组织数据
...
用栈实现逆波兰算法就非常合适，也许有其他的实现，但是肯定会很折腾，用合适的数据结构去描述合适的问题是一个程序员非常重要，因为底层数据结构的设计直接影响到上层代码的实现，如果你发现你的代码写的很复杂，想想是不是数据结构设计错了。

...
### 元组（tuple）
元组（tuple）和列表（list）的功能类似，都可以用来表示有序数据的集合，区别 `list` 有新增元素、修改元素、删除元素等改变 `list` 的功能而 `tuple` 不能进行任何修改。
...
逆波兰表示法（[wiki 解释](https://zh.wikipedia.org/wiki/逆波兰表示法)）：我们常用的数学表达式由于把运算符合放中间所以都属于中缀表达式，例如 `3 + 4`、`1 + 2`、`(1 + 2) * 3`，逆波兰表示法是将运算符合放在后面也叫做后缀表达式，例如 `3 4 +`、`1 2 +`、`1 2 + 3 *`，可以看出来逆波兰表示法很好的帮助我们去掉了括号，体现了运算的优先级。
